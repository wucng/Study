...
/*
 * 两种方式实现host-->cuArray,再cuArray-->纹理内存
...
void runTest(int argc, char **argv)
{
...
#if 1
    // hData-->dData-->outputSurface-->cuArray (hData-->cuArray host数据复制到cuArray)
    checkCudaErrors(cudaMemcpy(dData, hData, size, cudaMemcpyHostToDevice)); // hData-->dData
    checkCudaErrors(cudaBindSurfaceToArray(outputSurface, cuArray)); // outputSurface-->cuArray

    surfaceWriteKernel<<<dimGrid, dimBlock>>>(dData, width, height); // dData-->outputSurface
#else // This is what differs from the example simpleTexture
    // (hData-->cuArray host数据复制到cuArray)
    ...
    // Bind the array to the texture （cuAarray --> 纹理内存）
