// 直接编译：nvcc conv.cu -o conv -std=c++11 `pkg-config opencv --cflags --libs` -lcudnn
// or 使用Makefile文件 直接make
...
cv::Mat load_image(const char* image_path) {
  cv::Mat image = cv::imread(image_path, cv::IMREAD_COLOR); // or CV_LOAD_IMAGE_COLOR
  image.convertTo(image, CV_32FC3); // image.type()=CV_32FC3; 报错，image.type()不能修改，是常量类型
  cv::normalize(image, image, 0, 1, cv::NORM_MINMAX);// 转到0.～1.
  return image;
}
...
int main(int argc, const char* argv[]) {
	...
	cudaSetDevice(gpu_id); // 设置使用哪块GPU
