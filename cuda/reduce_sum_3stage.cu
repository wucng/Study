...
{
    mycout<<"归约算法求和 3阶段求和"<<endl;
    int N=100000000; // 使用3阶段 最多取值为   1024*1024*1<N<=1024*1024*1024
    ...
    // 阶段二
    int grid2=ceil(1.0*N2 / num_bs);
    int N3=grid2;

    // 阶段三汇总
    int grid3=ceil(1.0*N3 / num_bs);


    int n2bytes = N2 * sizeof(FLOAT); // 中间结果
    int n3bytes = N3 * sizeof(FLOAT); // 中间结果
    int onebytes = 1 * sizeof(FLOAT); // 最终结果

    /**======1、CPU 创建变量并赋值初始化==========*/
    FLOAT *dev_x=NULL,*host_x=NULL;
    FLOAT *dev_y=NULL; // 阶段一临时中间变量
    FLOAT *dev_y2=NULL; // 阶段二临时中间变量
    ...
    HANDLE_ERROR(cudaMalloc((void **)&dev_y, n2bytes));
    HANDLE_ERROR(cudaMalloc((void **)&dev_y2, n3bytes));
    ...
    {
        // stage 2
        global_reduce_sum<<<grid2,bs>>>(dev_y,dev_y2,N2);
        // share_reduce_sum<<<grid2,bs,bs*sizeof(FLOAT)>>>(dev_y,dev_y2,N2);
        // share_reduce_sum2<<<grid2,bs,bs*sizeof(FLOAT)>>>(dev_y,dev_y2,N2);
    }

    {
        // stage 3
        global_reduce_sum<<<grid3,bs>>>(dev_y2,dev_z,N3);
        // share_reduce_sum<<<grid3,bs,bs*sizeof(FLOAT)>>>(dev_y2,dev_z,N3);
        // share_reduce_sum2<<<grid3,bs,bs*sizeof(FLOAT)>>>(dev_y2,dev_z,N3);
    }
