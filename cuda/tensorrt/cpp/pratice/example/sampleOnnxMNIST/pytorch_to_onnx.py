...
class MnistModel(object):
    def __init__(self):
        self.batch_size = 64
        self.test_batch_size = 32#100
        self.learning_rate = 0.0025
        self.sgd_momentum = 0.9
        self.log_interval = 100
        self.data_path="/media/wucong/work/practice/work/data"
        # Fetch MNIST data set.
        self.train_loader = torch.utils.data.DataLoader(
            datasets.MNIST(self.data_path, train=True, download=False, transform=transforms.Compose([
                transforms.ToTensor(),
        ...
        self.test_loader = torch.utils.data.DataLoader(
            datasets.MNIST(self.data_path, train=False, transform=transforms.Compose([
                transforms.ToTensor(),
                # transforms.Normalize((0.1307,), (0.3081,))
                ])),
            ...
            shuffle=True)
        self.network = Net()
        self.is_gpu= True  if torch.cuda.is_available() else False
        if self.is_gpu:
    ...
    def saveONNX(self, filepath):
        ...
        # 神经网络输入数据类型
        # dummy_input = torch.randn(self.batch_size, 1, 28, 28, device='cuda')
        dummy_input = torch.randn(self.batch_size, 1, 28, 28).to("cuda")
        torch.onnx.export(self.network, dummy_input, filepath, verbose=True,export_params=True)
...
if __name__=="__main__":
    ...
    # save model
    dict_weights=m.get_weights()
    torch.save(dict_weights,os.path.join(dir_path,"model.pt"))
    ...
    np.savez(os.path.join(dir_path,"model.npz"), **weights_arg)

    m.saveONNX(os.path.join(dir_path,"model.onnx"))
