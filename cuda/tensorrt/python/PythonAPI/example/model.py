...
if __name__=="__main__":
    ...
    if not os.path.exists("model.pt"):
        m.learn(num_epochs=1)

        # save model
        dict_weights=m.get_weights()
        torch.save(dict_weights,"model.pt")
        # torch.save(m.network,"all_model.pt") # 保存整个模型包括模型参数
        # model=torch.load("all_model.pt")
    else:
        dict_weights = m.get_weights()
        m.network.load_state_dict(torch.load("model.pt"))
        # save to npz
        weights_arg = {}
