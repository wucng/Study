...
def gpu_softmax(g_x,x_shape, threads_per_block=256):
    mod = SourceModule("""
...
def gpu_relu_2d(g_x,x_shape,threads_per_block=256):
    """
    :param x: [-1,10]
    ...
    block=(threads_per_block,1,1)
    grid = (x_shape[0]*x_shape[1]//threads_per_block+1,1,1)
    func(g_x,g_shape, grid=grid, block=block)
    # x = cuda.from_device(g_x, x.shape, x.dtype)

    return g_x,x_shape

def gpu_relu_4d(g_x,x_shape):
    """
    :param x: [-1,24,24,20]
    :return:
    """
...
def gpu_BN(g_x,x_shape,gamma,beta,mean,var,esp=1e-3):
    """
    :param x: [-1,24,24,20]
    :param gamma:[20,]
    :param beta:[20,]
...
def conv2d(g_x,x_shape,weight,bias):
    """
    ...
    return g_out2,out2.shape
    # return out[:,2:x_shape[1]-2,2:x_shape[2]-2,:]

def maxpool(g_x,x_shape):
    """
    x:[-1,24,24,20]
    out:[-1,24,24,20]
    s:2x2
    ...
    # x_shape = x.shape
    out1 = np.zeros([x_shape[0], x_shape[1], x_shape[2], x_shape[3]], np.float32)
    out = np.zeros([x_shape[0], x_shape[1]//2, x_shape[2]//2, x_shape[3]], np.float32)
    ...
    return g_out,out.shape

def gpu_reshape(g_x,x_shape):
    """
...
def gpu_matrix_mul_add_bias(g_x,x_shape, wights, bias=None, threads_per_block=256):
    mod = SourceModule("""
...
def model(parmas,img):
    ...
    x_shape = x.shape
    g_x = cuda.to_device(x)
    g_x,x_shape=conv2d(g_x,x_shape,parmas["conv2d/kernel:0"],parmas["conv2d/bias:0"])
    g_x,x_shape = gpu_BN(g_x,x_shape, parmas["batch_normalization/gamma:0"], parmas["batch_normalization/beta:0"],
    parmas["batch_normalization/moving_mean:0"], parmas["batch_normalization/moving_variance:0"])
    g_x,x_shape=gpu_relu_4d(g_x,x_shape)
    g_x,x_shape=maxpool(g_x,x_shape)

    g_x,x_shape=conv2d(g_x,x_shape,parmas["conv2d_1/kernel:0"],parmas["conv2d_1/bias:0"])
    g_x,x_shape = gpu_BN(g_x,x_shape, parmas["batch_normalization_1/gamma:0"], parmas["batch_normalization_1/beta:0"],
    parmas["batch_normalization_1/moving_mean:0"], parmas["batch_normalization_1/moving_variance:0"])
    g_x,x_shape = gpu_relu_4d(g_x,x_shape)
    g_x,x_shape=maxpool(g_x,x_shape)

    g_x,x_shape=gpu_reshape(g_x,x_shape)

    g_x,x_shape=gpu_matrix_mul_add_bias(g_x,x_shape,parmas['dense/kernel:0'],parmas['dense/bias:0'])
    g_x,x_shape=gpu_relu_2d(g_x,x_shape)
    g_x,x_shape=gpu_matrix_mul_add_bias(g_x,x_shape,parmas['dense_1/kernel:0'],parmas['dense_1/bias:0'])

    # softmax
    g_x,x_shape =gpu_softmax(g_x,x_shape)

    # from GPU -->CPU
    x = cuda.from_device(g_x, x_shape, x_dtype)
    return x
